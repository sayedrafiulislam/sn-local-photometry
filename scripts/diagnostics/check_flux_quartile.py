...
def col(tag):
    ...
    df = df[(df.radius_kpc==5.0) & df.annulus_ok & df.aperture_ok & (df.telescope=="dup")]
...
j["shift"] = (j.c_a - j.c_b) * 1000
q = pd.qcut(j.fB_b, 4, labels=["faintest","Q2","Q3","brightest"])
print(j.groupby(q, observed=True).shift.agg(["count","median","mean"]).round(1))
print("\nSpearman shift vs flux:", j.shift.corr(j.fB_b, method="spearman").round(3))